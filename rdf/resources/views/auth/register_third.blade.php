<x-guest-layout>
    ...
    <div id="finishing" class="hidden fixed flex h-full left-0 top-0 w-full z-40 items-center justify-center">
        <div class="flex flex-col items-center w-full lg:w-1/2 h-3/4 bg-black/30 backdrop-blur-[30px] z-50 p-5">
            ...
            <p class="text-white/60 text-center text-sm lg:text-2xl mt-5 lg:mt-10">{{ $page_texts->where('title', 'Account Created')->first()->content ?? ''}}</p>
    ...
    <div class="lg:flex lg:flex-row-reverse gap-10 lg:mt-20 lg:mx-[120px] mt-10 mx-5">
        ...
        <div class="flex lg:flex-1 flex-col space-y-10">
            <div class="flex items-center">
                ...
                <h3 class="text-[15px] lg:text-xl text-primary ml-3 uppercase font-bold">Create your password</h3>

            </div>
            <div class="flex items-center">
                <h2 class="head_line relative font-bold text-xl lg:text-[40px] text-white">Just One more step</h2>
            ...
            <div class="pb-10 lg:pb-0 lg:pr-[170px]">
                <form
                    method="POST"
                    action="{{ route('password', ['email' => $email]) }}"
                    id="passwordForm"
                    data-email="{{ $email }}"
                    ...
                    <x-primary-button type="submit" disabled id="submitPassword" contentClasses="bg-primary-class mt-12 lg:py-[10px] border-none w-full text-white hover:text-white hover:bg-white/[0.13]">
                        <span class="font-extralight lg:text-lg">Submit</span>
                    </x-primary-button>
                </form>
            </div>
        </div>
